    ...
    <div className="p-6 flex gap-6">
      <div className="flex-1 min-w-0">
        <div className="flex items-center justify-between mb-5">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => setMonth(m => subMonths(m, 1))} className="btn-secondary p-2"><ChevronLeft size={16} /></button>
            <button onClick={() => setMonth(new Date())} className="btn-secondary text-xs px-3 py-2">Today</button>
            <button onClick={() => setMonth(m => addMonths(m, 1))} className="btn-secondary p-2"><ChevronRight size={16} /></button>
          </div>
        </div>
              ...
              {days.map((day, i) => {
                ...
                return (
                  <div
                    ...
                    className={[
                      "min-h-[90px] p-2 border-b border-r border-gray-800 cursor-pointer transition-colors",
                      !inMonth && "opacity-30",
                  ...
                  >
                    ...
                    <div className="mt-1 space-y-0.5">
                      {dayTasks.slice(0, 3).map(t => (
                        <div key={t.id} className="flex items-center gap-1 overflow-hidden" onClick={e => { e.stopPropagation(); setModalTask(t); }}>
                          <div className={`w-1.5 h-1.5 rounded-full shrink-0 ${STATUS_COLORS[t.status]}`} />
                          <span className="text-[11px] text-gray-300 truncate">{t.title}</span>
                        </div>
                      ))}
                      {dayTasks.length > 3 && (
                        <span className="text-[10px] text-gray-500">+{dayTasks.length - 3} more</span>
                      )}
                    </div>
                  </div>
                );
        ...
        <div className="w-72 shrink-0">
          <div className="sticky top-6 space-y-3">
            <div className="flex items-center justify-between">
              <h2 className="text-sm font-semibold text-white">{format(selectedDay, "MMMM d, yyyy")}</h2>
              <button onClick={() => setModalTask("new")} className="btn-primary px-2.5 py-1.5 text-xs">
                <Plus size={13} />
                Task
              </button>
            </div>
            {selectedTasks.length === 0 ? (
              <div className="card text-center py-8">
                ...
                <button onClick={() => setModalTask("new")} className="text-xs text-brand-400 hover:text-brand-300 mt-2">+ Create one</button>
              </div>
            ) : (
                ...
                {selectedTasks.map(t => (
                  ...
                  >
                    <div className="flex items-start gap-2">
                      <div className={`w-2 h-2 rounded-full shrink-0 mt-1.5 ${STATUS_COLORS[t.status]}`} />
                      <div className="min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-2">
                          <span className={`badge border text-[10px] ${STATUS_BADGE[t.status]}`}>{t.status.replace("_", " ")}</span>
                          <span className="text-[11px] text-gray-500">{format(t.dueAt, "HH:mm")}</span>
                        </div>
                        {t.assignedWorkerEmail && (
                          <p className="text-[11px] text-gray-500 mt-1">@{t.assignedWorkerEmail.split("@")[0]}</p>
                        )}
                      </div>
                    </div>
                  </div>
      ...
      {modalTask !== null && (
        <TaskModal
          ...
          defaultDueAt={selectedDay ? new Date(selectedDay.toDateString() + " 09:00").getTime() : undefined}
          onSave={onSave}
